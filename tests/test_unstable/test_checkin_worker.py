...
def test_flush_and_checkin(
    ...
    worker.report_error(
        Error(level=ErrorLevel.error, description=faker.sentence(), task_name="task1", extractor=test_extractor)
    )
...
def test_run_report_periodic_ensure_reorder(
    ...
    ahead = now()
    first_error = Error(
        level=ErrorLevel.error, description=faker.sentence(), task_name="task1", extractor=test_extractor
    )
    second_error = Error(
        level=ErrorLevel.warning, description=faker.sentence(), task_name="task1", extractor=test_extractor
    )
